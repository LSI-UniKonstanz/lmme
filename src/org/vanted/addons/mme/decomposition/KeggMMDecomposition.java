...
public class KeggMMDecomposition extends MMDecompositionAlgorithm {
	...
	private String[] requestPackage(ArrayList<Node> reactionPackage) {
		...
		for (Node reactionNode : reactionPackage) {
			String keggId = MMEController.getInstance().getCurrentSession().getNodeAttribute(reactionNode,
		...
		return response.split(Pattern.quote(MMEConstants.KEGGSEP));
	}
			...
			while (lineIndex < arrLines.length) {
				if (arrLines[lineIndex].startsWith("PATHWAY"))
					do {
						...
						if (!MMEConstants.INEGLIGIBLE_KEGG_PATHWAYS.contains(line)) {
	...
	private void extractNodesFromHugestSubsystem() {
		...
		for (Node reactionNode : node2possibleSubsystems.keySet()) {
			if (node2possibleSubsystems.get(reactionNode).contains(hugestSubsystem)) {
				nodesToRemove.add(reactionNode);
				MMEController.getInstance().getCurrentSession().addNodeAttribute(reactionNode,
						ATTRIBUTE_NAME_FINAL_SUBSYSTEM, hugestSubsystem);
			}
		}
		MMEController.getInstance().getTab()
	...
	public FolderPanel getFolderPanel() {
		...
		if (this.fp != null) {
			...
			JPanel separatorLine = MMETab.combine(lblSeparator, this.separator, Color.WHITE, false, true);
			...
			JComponent minimumNumberComponent = TableLayout.getDoubleRow(
					MMETab.combine(lblMinimumNumber, this.minimumNumberValue, Color.WHITE, false, true),
