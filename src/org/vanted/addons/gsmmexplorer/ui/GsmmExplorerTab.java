...
public class GsmmExplorerTab extends InspectorTab {
	...
	public GsmmExplorerTab() {
		...
		btnSetModel.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				GsmmExplorerController.getInstance().setModelAction();
			}
		});
		...
		btnShowOverviewGraph.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				GsmmExplorerController.getInstance().showOverviewGraphAction();
			}
		});
		...
		btnShowSubsystems.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				GsmmExplorerController.getInstance().showSubsystemGraphsAction();
			}
		});
		...
		btnTransformToSbgn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				GsmmExplorerController.getInstance().transformToSbgnAction();
			}
		});

		GsmmExplorerController.getInstance().setTab(this);
	}
...
//		addOneElementSpanning(monitorOuterPanel, fpSessionInfo);
	...
	private JComponent createSettingsComponent() {
		...
		Set<String> decompositionAlgorithms = GsmmExplorerController.getInstance().getDecompositionAlgorithmsMap()
				.keySet();
		...
		cbDecompMethod.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				GsmmDecompositionAlgorithm decompositionAlgorithm = GsmmExplorerController.getInstance()
		...
		this.ckbSplitDefaultSubS
				.setToolTipText("<html>If ticked, the connected components of the default subsystem are computed <br> and treated "
		...
		//fpSettings.addGuiComponentRow(FolderPanel.getBorderedComponent(ckbSplitDefaultSubS, 0, 0, 0, 0), null, true);

		//JLabel lblSplitDefaultSubSThreshold = new JLabel("Threshold:");
		this.tfSplitDefaultSubSThreshold = new JTextField(5);
		JPanel pSplitDefaultSubSThreshold = combine(ckbSplitDefaultSubS, tfSplitDefaultSubSThreshold,
		...
		Set<String> overviewLayouts = GsmmExplorerController.getInstance().getOverviewLayoutsMap().keySet();
		...
		this.cbOverviewLayout.setSelectedItem(ForceDirectedGsmmLayout.name());
	...
	private JComponent createCloningComponent() {
		...
		sliderSplitDeg.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				...
				if (GsmmExplorerController.getInstance().getCurrentSession().getBaseGraph() != null) {
					labelSliderCorrespSpeciesNumber.setText(String.valueOf(GsmmExplorerController.getInstance()
		...
		editCloneListButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				} else if (GsmmExplorerController.getInstance().getCurrentSession().getBaseGraph() != null) {
					ArrayList<Node> speciesAbove = GsmmExplorerController.getInstance().getCurrentSession()
							.getBaseGraph().getOriginalSpeciesWithDegreeAtLeast(sliderSplitDeg.getValue());
...
//					}
	...
	private JComponent createSubsystemsViewComponent() {
		...
		Set<String> subsystemLayouts = GsmmExplorerController.getInstance().getSubsystemLayoutsMap().keySet();
		...
		this.cbSubsystemLayout.setSelectedItem(ForceDirectedGsmmLayout.name());
	...
	public ArrayList<Node> getClonableSpecies() {
		...
		if (editedCloneList) {
			...
			return GsmmExplorerController.getInstance().getCurrentSession().getBaseGraph()
	...
	public void updateGUI() {
		...
		GsmmExplorerSession session = GsmmExplorerController.getInstance().getCurrentSession();
