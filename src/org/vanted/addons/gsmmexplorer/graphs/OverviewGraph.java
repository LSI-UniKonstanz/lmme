...
public class OverviewGraph {
	...
	private GsmmDecomposition decomposition;
	...
	public OverviewGraph(GsmmDecomposition decomposition) {
		...
		for (Edge e : GsmmExplorerController.getInstance().getCurrentSession().getBaseGraph().getGraph().getEdges()) {
			edges++;
	...
	private void determineInterfaces() {
		...
		for (Node speciesNode : GsmmExplorerController.getInstance().getCurrentSession().getBaseGraph()
				.getSpeciesNodes()) {
	...
	public GsmmDecomposition getDecomposition() {
