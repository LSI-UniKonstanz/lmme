	...
	protected ArrayList<SubsystemGraph> runSpecific() {
		...
		for (Set<Node> nodes : connComps) {
			for (Node node : nodes) {
				...
				if (GsmmExplorerTools.getInstance().isReaction(originalNode)) {
					GsmmExplorerController.getInstance().getCurrentSession().addNodeAttribute(originalNode,
		...
		return determineSubsystemsFromReactionAttributes(this.ATTRIBUTE_NAME_SUBSYSTEM, ";");
	}
